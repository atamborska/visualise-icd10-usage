...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      textInput("icd10_code", "Enter the code:", value = "A000"),
      sliderInput("year_range", "Select year range:",
                  min = 2013, max = 2023, value = c(2013, 2023), step = 1)
    ),
...
server <- function(input, output) {
  ...
  filtered_df <- reactive({
    req(input$icd10_code)
    df |>
      filter(
        code == input$icd10_code,
  ...
  output$usage_plot <- renderPlot({
      ggplot(filtered_df, aes(x = year, y = use)) +
        geom_line() +
        geom_point() +
        labs(x = "Year", y = "Usage") +
        theme_minimal()
...
reticulate::repl_python() Please note that for this file to knit you have to install streamlit (pip install streamlit) & matplotlib.pylot (pip )
